...
export const CALENDAR_WIDGETS = [
  { id: "stat_month_pnl",  label: "Month P&L",         category: "stats" },
  { id: "stat_winrate",    label: "Win Rate",           category: "stats" },
  { id: "stat_streak",     label: "Win Streak",         category: "stats" },
  { id: "stat_best_day",   label: "Best Day",           category: "stats" },
  { id: "stat_worst_day",  label: "Worst Day",          category: "stats" },
  { id: "stat_active_days",label: "Active Days",        category: "stats" },
  { id: "stat_trades",     label: "Total Trades",       category: "stats" },
  { id: "cal_stats",       label: "Month Summary",      category: "panels" },
  { id: "top_symbols",     label: "Top Symbols",        category: "panels" },
  { id: "equity_curve",    label: "Equity Curve",       category: "charts" },
  { id: "profit_chart",    label: "Profit Chart",       category: "charts" },
...
const CATS = ["all", "stats", "panels", "charts", "tables"];

export default function AddCalendarWidgetSidebar({ activeWidgets, onAdd, onClose }) {
  const [search, setSearch] = useState("");
  const [cat, setCat] = useState("all");

  const filtered = CALENDAR_WIDGETS.filter(w => {
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-50 w-80 bg-card border-l border-border shadow-2xl flex flex-col">
      <div className="flex items-center justify-between px-5 py-4 border-b border-border">
        <div>
          <h2 className="font-heading font-bold text-foreground text-base">Calendar Widgets</h2>
          <p className="text-xs text-muted-foreground mt-0.5">Add panels to your calendar view</p>
        </div>
